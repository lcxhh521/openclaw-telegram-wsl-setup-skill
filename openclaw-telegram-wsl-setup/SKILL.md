...
- The control center should use `openclaw channels status --json --timeout 30000` in addition to `gateway probe` so Telegram cold-start and message flow are visible. Surface `lastStartAt`, `lastInboundAt`, and `lastOutboundAt` as states such as `启动中`, `连接中`, `已收到未回复`, and `已回复`; this helps distinguish model/agent latency from Telegram polling or gateway startup failure.
