...
- 文章深读子页采用“原文定位短摘 + 逐段解析 + 全文深度解读”的结构，便于原文和解析对照阅读。
- 用 `people_daily_publications.json` 和 Notion 端同名页检查共同防重复；默认不重复创建同一天页面，只有显式 `--force` 才归档旧页并重建。
